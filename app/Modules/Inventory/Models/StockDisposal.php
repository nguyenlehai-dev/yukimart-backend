...
class StockDisposal extends Model
{
    ...
    protected static function booted(): void
    {
        static::creating(function (self $m) {
            if (empty($m->code)) {
                $m->code = 'XH'.str_pad((self::max('id') ?? 0) + 1, 6, '0', STR_PAD_LEFT);
            }
            $m->created_by = $m->created_by ?? auth()->id();
            $m->updated_by = $m->updated_by ?? auth()->id();
            $m->disposal_date = $m->disposal_date ?? now();
        });
    ...
    public function scopeFilter($query, array $f)
    {
        return $query
            ...
            ->when($f['from_date'] ?? null, fn ($q, $v) => $q->whereDate('disposal_date', '>=', $v))
            ->when($f['to_date'] ?? null, fn ($q, $v) => $q->whereDate('disposal_date', '<=', $v))
